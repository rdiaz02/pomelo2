...
## necesitamos leer los datos y preparar el survival object
...
cox.parallel <- function(x, time, event, MaxIterationsCox = 200) { 
    ...
    res.mat[is.na(res.mat[, 2]), c(2, 4)] <- 999
    colnames(res.mat) <- c("coeff", "p.value", "Warning", "FDR")
    return(res.mat)
}

rescox <- cox.paralel(xdata, Time, Event, MaxIterationsCox = 200)  

p.values.original <- data.frame(
                                Names = geneNames,
                                Row = 1:length(geneNames),
                                p.value = rescox[, 2],
                                FDR.adj.p.value = rescox[, 4],
                                coef = rescox[, 1], 
                                abs.coeff = abs(rescox[, 1]),
                                Warning = rescox[, 3])

p.values.original <-
    p.values.original[order(p.values.original$FDR.adj.p.value,
                            -p.values.original$abs.coeff), ]
...
#### lanzar como:
tryrrun = os.system('/http/mpi.log/tryRrun2.py ' + tmpDir +' 10 ' + 'PomeloII &')
