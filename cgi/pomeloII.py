...
#covarNoNames = open(tmpDir + '/covarNoNames', mode = 'w')
covarR = open(tmpDir + '/covarR', mode = 'w')

all_covar_lines = srvfile.read().splitlines()
srvfile.close()

for nr in range(1, len(all_covar_lines)):
...
if test_type == 't' or test_type == 'FisherIxJ' or test_type =='Anova':
    ...
    if os.path.exists(tmpDir + '/errorInput'):
        commonOutput()
        rif = open(tmpDir + '/errorInput', type = 'r')
        rift = rif.read()
