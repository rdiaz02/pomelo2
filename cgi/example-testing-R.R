### Just output one single diagnostic number per node. Create a "tmp.sh" file
###  as


##############3
#!/bin/sh                                                                                              
#cd /http/pomelo2/www/tmp/449-e                                                                         
#/var/www/bin/R-local-7-LAM-MPI/bin/R --no-readline --no-save --slave < f2.R > f2.Rout

###########

### and execute by doing for a in `seq -w 1 30`; do ssh karl$a "/http/tmp.sh"; done
...
geneNames <- read.table("geneNames",quote="", stringsAsFactors = TRUE)[, 1]
...
options(digits = 22)
rescox2[c(76), ]
nombre <- system("hostname", inter = TRUE)
write(file = paste("VALORES", nombre, sep = "."), rescox2[76, ])
...
c1 <- paste(nombre, rescox2[76, 1], sep = ":   ")
c2 <- paste(nombre, rescox2[316, 1], sep = ":   ")
c3 <- paste(nombre, rescox2[790, 1], sep = ":   ")
...
## rescox2[c(76, 316, 790), ]
##                    coeff            p.value Warning                FDR
## [1,] 0.28680765341456094 0.1284896356324415       0 0.4310664805158096
## [2,] 0.03022587627234304 0.8471165080043861       0 0.9451302456327552
## [3,] 0.02151198573535192 0.8637957669427898       0 0.9524796075292293
## > 
...
## write.table(file = "multest_parallel.res2",
