def covars_header_results(covar_filename, header_filename, results_filename):
	...
	for line in lines:
		...
		line = line.replace("\t \t","\tnan\t") ## R.D.-U.
		line = line.replace("\t\n","\tnan\n")## R.D.-U.
		parts = line.split("\t")
		parts = line.split()
		gene_names.append(parts[0])
		print parts[1:]  ##debugging; R.D.-U.
		covar.append(map(float, parts[1:])) 
#  	covarIn = open('outcovarR', mode = 'r')
# 	lines = covarIn.readlines()
# 	for line in lines:
# 		parts = line.split("\t")
# 		covar.append(map(float, parts[1:]))
# 	covarIn.close()
...
def filter_covars(covar, gene_names, results, result_row_id, cgi_dict, def_max_genes):
	...
	for row, loop_row_ID in zip(results_loop, result_row_id):
		if dict.has_key(row [1]):
			dict[row [1]] = dict[row [1]] + [loop_row_ID]
		else:
			dict[row [1]] = [loop_row_ID]
			FDR_array.append(row [1])
	FDR_array.sort()
